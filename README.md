...
export type NuevaCompra = unknown; // TODO: qué se necesita para crear la transacción
```
...
export const fuenteMemoria: FuenteDatos = {
  listarFunciones: pendiente,
  listarClientes: pendiente,
  crearCliente: pendiente,
  listarCompras: pendiente,
  crearCompra: pendiente,
  cancelarCompra: pendiente,
};
```
...
Corre en `http://localhost:3000` y expone: `GET /funciones`, `GET|POST /clientes`,
`GET|POST /compras`, `PATCH /compras/:id`. Visite `http://localhost:3000/`
